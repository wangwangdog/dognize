...
_CORE_CHANLUN_SRC = str(BASE_DIR / "core" / "chanlun")
...
def _run_orig_cl_subprocess(symbol: str, df: pd.DataFrame, freq: str) -> dict:
    ...
    script = '''
import sys, json
sys.path.insert(0, "''' + _CORE_CHANLUN_SRC + '''")

from cl import CL as ChanlunCL
import pandas as pd
...
def _run_cl(symbol: str, df: pd.DataFrame, freq: str, config_extra: dict = None) -> dict:
    ...
    _CL = _get_cl()
    cd = _CL(symbol, freq, config=config_extra or {})
