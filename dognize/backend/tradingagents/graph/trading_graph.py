...
class TradingAgentsGraph:
    ...
    def _send_progress_update(self, chunk, progress_callback):
        """发送进度更新到回调函数（仅传 Agent 名称，内容由 final_state 提取）"""
        try:
            ...
            if '__end__' in chunk:
                progress_callback(json.dumps({"agent": "完成", "content": ""}, ensure_ascii=False))
                return
            ...
            node_mapping = {
                ...
                'tools_market': None, 'tools_fundamentals': None,
                'tools_news': None, 'tools_social': None,
                'Msg Clear Market': None, 'Msg Clear Fundamentals': None,
                'Msg Clear News': None, 'Msg Clear Social': None,
            ...
            name = node_mapping.get(node_name)
            if name is None:
                return

            # 只传 Agent 名称，表示进度到了哪一步
            import json
            progress_callback(json.dumps({"agent": name, "content": ""}, ensure_ascii=False))
